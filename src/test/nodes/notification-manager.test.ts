...
describe("inovelli-notification-manager", () => {
    ...
    helper.load(notificationManagerNode, flow, () => {
      ...
      n2.on("input", (msg: any) => {
        try {
          ...
          assert.strictEqual(msg.payload.data.value, 10 + 50 * 256); // rainbow=10, intensity 50 (5*10)
          done();
        } catch (err) {
          done(err);
        }
  ...
  // Regression test: same reasoning as LED Manager's equivalent test - a
  // fractional brightness payload override must round to an integer before
  // it's baked into the bitpacked notification value.
  it("rounds a fractional brightness payload override to the nearest integer", (done) => {
    ...
    helper.load(notificationManagerNode, flow, () => {
      ...
      n2.on("input", (msg: any) => {
        try {
          // brightness 7.6 -> rounds to 8 -> value = 0 + 8*256 + 10*65536 + 4*16777216
